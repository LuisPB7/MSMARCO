...
with torch.no_grad():
    with open('top{}.dev.sorted.tsv'.format(K), 'wt', encoding='utf-8') as out_file:
        ...
        data = DataLoader(LuceneDataset(topK_dev), shuffle=True, num_workers=8, batch_size=256, collate_fn=PadSequence())
        for batch in data:
            qids = batch[0]
            pids = batch[1]
